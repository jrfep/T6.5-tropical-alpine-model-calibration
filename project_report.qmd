...
Many of the areas studied showed evidence of human transformation, meaning they are likely an invalid data point. Additionally, the majority of ranges show the presence of other ecosystems within the typology, however this does not necessarily mean T6.5 cannot be found in the range.
The majority of ranges studied have more information present in papers that will require further time spent searching for relevant factors, however many do not have sufficient information present and currently accessible to make a valid conclusion.
...
This project has focussed on Tropical Alpine Grasslands and Herblands, finding for each location any observations pointing towards their presence, and if not, the possibility that the given areas have been transformed (likely through human interaction).
...
The number of ranges analysed per region is shown below
...
The table below contains a list of references, sorted by region, range and search term.
...
```
  ...
  The table below shows the number of references found in each given region.
...
```
  ...
  Graph of the average number of references found per range for each region
...
Graph of average number of references per given search term
```{r, warning=FALSE, message=FALSE}
...
barplot(search_reference_density$search_reference_density,
        ...
        names.arg=c("other","ecosystems","flora and fauna","climate"))
...
Many ranges were found to potentially contain T6.5 ecosystems, however there were very few with guaranteed observations. Many of the ranges have evidence for transformation and many have evidence for other ecosystems present. More research is needed to find complete results to allow for model calibration.
